...
void ext2_ensure_root_dir_loaded()
{
    if (!root_dir_loaded) {
...
struct dir_entry *ext2_lookup_root_dir(const char *d_name) 
{
    int i = 0;

    ext2_ensure_root_dir_loaded();
...
void ext2_get_file_stat(struct dir_entry *dentry, int *size, int *block_count)
{
    int i;
...
struct file_info *ext2_load_file(struct dir_entry *dentry)
{
...
void ext2_remove_file(struct dir_entry *dentry)
{
    int i;
...
int ext2_commit_file(struct file_info *fi)
{
...
void ext2_get_root_dir_info(int *file_count, int *free_dentries)
{
    int i = 0;
...
int ext2_getattr(const char *path, struct stat *statbuf) 
{
    ...
    if ( (path_len == 1) && path[0] == '/') {
        ...
        struct dir_entry *dentry = ext2_lookup_root_dir(&path[1]);
        int size, block_count;
        
        if (dentry == NULL)
            return -ENOENT;
        
        ext2_get_file_stat(dentry, &size, &block_count);
...
int ext2_mknod(const char *path, mode_t mode, dev_t dev) 
{
    ...
    if ( S_ISREG(mode) ) {
        ...
        de = ext2_lookup_root_dir(f_name);
        if (de != NULL)
            return -EEXIST;
...
{
    struct dir_entry *de = ext2_lookup_root_dir(&path[1]);
...
int ext2_open(const char *path, struct fuse_file_info *fileInfo) 
{
    ...
    struct file_info *fi = ext2_load_file(de);
...
int ext2_grow_file(struct file_info *fi, int new_size)
{
    struct dir_entry *de = fi->f_dentry;
    int i, extra_size;
    ...
    while (extra_size > 0) {
        int sector = ext2_get_free_block();
        if (sector == -1) {
            return -ENOSPC;
        }
        map[sector] = 0xFF;
...
int ext2_write(const char *path, const char *buf, size_t size, off_t offset, struct fuse_file_info *fileInfo) 
{
    ...
    ext2_get_file_stat(fi->f_dentry, &f_size, &f_bcount);
    ...
    if (offset + size > fi->f_size) {
        ...
        int ret = ext2_grow_file(fi, new_size);
...
int ext2_statfs(const char *path, struct statvfs *statInfo) 
{
    ...
    ext2_get_root_dir_info(&file_count, &free_dentries);
    ...
    statInfo->f_bfree = ext2_count_free_blocks();    /* # free blocks */
    statInfo->f_bavail = ext2_count_free_blocks();   /* # free blocks for unprivileged users */
